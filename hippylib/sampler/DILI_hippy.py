...
#             w=sp.linalg.eig(A,b=B,left=False,right=False)
        ...
        Omega=MultiVector(self.v, k+10)
        self.randomGen.normal(1., Omega)
        # for i in xrange(Omega.nvec()):
        #     Random.normal(Omega[i], 1., True)
        eigs=doublePassG(A,B,invB,Omega,k=k) # only need eigenvalues (correct)
        w=eigs[0]
...
#         self.Theta_r.export(self.model.problem.Vh[PARAMETER],os.path.join(self.savepath,'gLIS_'+self.filename+'.pvd'))
        self.Theta_r.export(self.model.problem.Vh[PARAMETER],os.path.join(self.savepath,'gLIS_'+self.filename+'.xdmf'))