...
class HessianAverage:
    ...
    def __init__(self, model, particle, options, x_all, comm):
        self.model = model
        self.particle = particle
        self.x_all = x_all
...
class Variation:
    ...
    def update_bases(self, particle):
        ...
        if self.options["type_projection"] is "hessian":
            hessian_misfit = HessianAverage(self.model, particle, self.options, self.x_all, self.comm)
    ...
    def update(self, particle, iter=0, step_norm=1):
        ...
        if self.options["is_projection"]:
            ...
            # self.is_bases_updated = False
            # if np.mod(iter, particle.coefficient_dimension) == 0 or (step_norm < self.options["step_projection_tolerance"]):  # np.mod(iter, particle.coefficient_dimension)
            #     if iter - self.iter_old == 1 and particle.coefficient_dimension < self.rank_Hessian_average and self.options["add_dimension"] > 0:
            #         particle.update_dimension(self.rank_Hessian_average)
            #         self.particle = particle
            #
            #     if self.rank == 0:
            #         print("update projection bases at iter " + str(iter) + " with step norm = " + str(step_norm)
    ...
    # gradient
            ...
            if self.options["type_approximation"] is "fisher":
                ...
                d = np.multiply(np.sqrt(np.abs(d)), np.sign(d))
        ...
        if self.options["type_approximation"] is 'fisher':
            ...
            if self.options["is_projection"]:
                ...
                if self.low_rank_Hessian_average:
                    ...
                    hessian_misfit = HessianAverage(self.model, self.particle, self.options, self.x_all, self.comm)
                hessian_misfit.mult(phat, pout)
