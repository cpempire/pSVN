...
if nproc == 1 and particle.number_particles == 1:
    ...
    for n in [2, 8, 32, 128, 512]:
        ...
        moment2 = model.generate_vector(PARAMETER)
        mhelp = model.generate_vector(PARAMETER)
        for m in range(n):
            mhelp.set_local(samples[m].get_local()**2)
            moment2.axpy(1. / n, mhelp)
        mhelp = model.generate_vector(PARAMETER)
        model.prior.M.mult(moment2, mhelp)
        ...
        qoi_moment2 = np.mean(np.array(qoi[:n])**2)
        cost_mean = np.mean(cost[:n])
        cost_std = np.std(cost[:n])
        cost_moment2 = np.mean(np.array(cost[:n])**2)

        print("for Laplace sampling at MAP with # samples = ", n,
              " mean L2-norm = ", meanL2norm, " pointwise variance L2-norm = ", varianceL2norm,
              "trace of covariance = ", trace, "qoi_mean = ", qoi_mean, "qoi_std", qoi_std,
              "cost_mean = ", cost_mean, "cost_std = ", cost_std)

        sample_statistics.append([n, meanL2norm, varianceL2norm, trace, moment2L2norm])
        qoi_statistics.append([n, qoi_mean, qoi_std, qoi_statistics])
        cost_statistics.append([n, cost_mean, cost_std, cost_moment2])
