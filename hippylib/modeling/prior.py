...
class _BilaplacianRsolver():
    ...
    def mult(self,x,y):
        # y = R^{-1} x
        self.Asolver.solve(self.help1, x)
        self.M.mult(self.help1, self.help2)
        self.Asolver.solve(y, self.help2)
...
class BiLaplacianPrior(_Prior):
    ...
    def sample_whiten(self, noise, s):
        """
        Sample a random function v(x) ~ N(0,I)
        vector v ~ N(0,M^(-1))
        """
        rhs = self.sqrtM * noise
        self.Msolver.solve(s, rhs)
...
class _FiniteDimensionalR:
    def __init__(self, sigma, dim, mesh):

        self.sigma = sigma
        self.CovInv = np.diag(1./(sigma.get_local()**2))
        self.dim = dim
        self.mesh = mesh
    ...
    def inner(self, x, y):
        ...
        return np.dot(y.get_local(), self.CovInv.dot(x.get_local()))

    def mult(self, x, y):

        y.set_local(self.CovInv.dot(x.get_local()))
...
class _FiniteDimensionalRsolver:
    ...
    def __init__(self, sigma, dim):
        ...
        self.sigma = sigma
        self.Cov = np.diag(sigma.get_local()**2)
        self.dim = dim
    ...
    def solve(self, x, b):
        x.set_local(self.Cov.dot(b.get_local()))

    def mult(self, x, y):
        y.set_local(self.Cov.dot(x.get_local()))
...
class FiniteDimensionalPrior:
    ...
    def __init__(self, Vh, mesh, sigma=None, mean=None):

        self.Vh = Vh
        self.mesh = mesh
        mvec = dl.Function(Vh).vector()
        self.dim = len(mvec.get_local())

        # a vector of standard deviations
        if sigma is None:
            self.sigma = dl.Function(Vh).vector()
            self.sigma.set_local(np.ones(self.dim))
        else:
            self.sigma = sigma
        ...
        if mean is None:
            ...
            self.mean.set_local(np.zeros(self.dim))
        else:
            self.mean = mean

        self.M = _FiniteDimensionalM(self.dim)
        self.Msolver = _FiniteDimensionalMsolver(self.dim)
        self.R = _FiniteDimensionalR(self.sigma, self.dim, self.mesh)
        self.Rsolver = _FiniteDimensionalRsolver(self.sigma, self.dim)
    ...
    def pointwise_variance(self, method, k = 1000000, r = 200):
        ...
        pw_var.set_local(self.sigma.get_local()**2)

        return pw_var

    def trace(self):

        return np.sum(self.sigma.get_local()**2)
